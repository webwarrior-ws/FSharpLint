...
let private isMatch index1 index2 (nodeArray:AbstractSyntaxArray.Node []) = 
    let numChildrenI = nodeArray.[index1].NumberOfChildren
    let numChildrenJ = nodeArray.[index2].NumberOfChildren

    if numChildrenI = numChildrenJ then
        let numChildren = numChildrenI

        { 0..numChildren } |> Seq.forall (fun child -> 
            index1 + child < nodeArray.Length && 
            index2 + child < nodeArray.Length && 
            nodeArray.[index1 + child].Hashcode = nodeArray.[index2 + child].Hashcode)
    else false
