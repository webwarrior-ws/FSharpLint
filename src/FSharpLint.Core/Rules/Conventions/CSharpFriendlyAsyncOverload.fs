...
let runner (args: AstNodeRuleParams) =
    let hasAsync (syntaxArray: array<AbstractSyntaxArray.Node>) nodeIndex fnIdent =
        let rec hasAsync index =
            if index >= syntaxArray.Length then
                ...
                match node with
                | AstNode.Binding (SynBinding (_, _, _, _, _attributes, _, _, pattern, _, _, range, _, _)) ->
    ...
    | AstNode.Binding (SynBinding (_, _, _, _, _, _, _, pattern, synInfo, _, range, _, _)) ->
        match synInfo with
        | Some (SynBindingReturnInfo (SynType.App(SynType.LongIdent(SynLongIdent(ident, _, _)), _, _, _, _, _, _), _, _, _)) ->
            match ident with
