...
let private runner (args: AstNodeRuleParams) =
    ...
    // Instead of examining current AST node, examine all nodes but only one time (when args.NodeIndex = 0).
    // This is done in order to have mutable variable failwithMessages local to the function and thus unique to
    // each examined source. Having this var module-level as before leads to occasional bugs when the rule is
    // run in parallel (e.g. in tests).
